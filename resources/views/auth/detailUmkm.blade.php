  ...
  <head>
    ...
    <style>
        ...
        .carousel-img {
            ...
            max-height: 400px;
            object-fit: contain;
            background-color: #f9f9f9; /* atau putih */
        }
    </style>
  </head>
          ...
          <div class="container">
            <div class="row">
                    ...
                    @if($umkm->items->count() > 0)
                        ...
                        <ol class="carousel-indicators">
                            ...
                            @foreach($umkm->items as $key => $item)
                                <li data-target="#umkmCarousel" data-slide-to="{{ $key }}" class="{{ $key == 0 ? 'active' : '' }}"></li>
                                @php
                                    $ii += 1;
                                @endphp
                            @endforeach
                            ...
                            @if ($umkm->video_file_path)
                                <li data-target="#umkmCarousel" data-slide-to="{{ $ii }}"></li>
                            @endif
                        </ol>
                        <div class="carousel-inner" role="listbox" style="max-height: 400px; overflow: hidden;">
                            @foreach($umkm->items as $key => $item)
                                <div class="item {{ $key == 0 ? 'active' : '' }}">
                                    <img src="{{ url('/storage/' . $item->umkm_file_path) }}" class="carousel-img" alt="Image {{ $key + 1 }}">
                                </div>
                            @endforeach
                            @if ($umkm->video_file_path)
                                <div class="item">
                                    <video class="carousel-img" controls muted autoplay>
                                        <source src="{{ url('/storage/' . $umkm->video_file_path) }}" type="video/mp4">
                                    </video>
                                </div>
                            @endif
                        </div>
                        <a class="left carousel-control" href="#umkmCarousel" role="button" data-slide="prev">
                            <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
                            <span class="sr-only">Previous</span>
                        </a>
                        <a class="right carousel-control" href="#umkmCarousel" role="button" data-slide="next">
              ...
              <div class="col-sm-6">
                <div class="row">
                  <div class="col-sm-12">
                    <h1 class="product-title font-alt">{{ $umkm->name ?? '-' }}</h1>
                  </div>
                </div>
                <div class="row mb-20">
                  <div class="col-sm-12">
                    <div class="price font-alt"><span class="amount">Rp {{ number_format($umkm->price) }}</span></div>
                  </div>
                </div>
                <div class="row mb-20">
                  <div class="col-sm-12">
                    <div class="description">
                        {!! $umkm->description ? nl2br(e($umkm->description)) : '-' !!}
                    </div>
                  </div>
                </div>
                <div class="row mb-20">
